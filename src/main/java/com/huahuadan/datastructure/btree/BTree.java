package com.huahuadan.datastructure.btree;
...
public class BTree {
    static class Node {
        ...
        public Node(int t) { // t>=2
            this.t = t;
            this.children = new Node[2 * t];//最大孩子数2t，约定
    ...
    void split(Node left, Node parent, int index) {
        ...
        //创建一个新的节点right，将left节点中较大key值拷贝到right中
        //如果是left是叶子节点，直接拷贝
        Node right = new Node(t);
        right.leaf = left.leaf;
        System.arraycopy(left.keys,t,right.keys,0,t-1);
        //如果left不是叶子节点，则需要将自己较大的孩子拷贝到right中
        if(!left.leaf){
    ...
    private void balance(Node parent, Node x, int i) {
        ...
        if (left != null) {
            ...
            x.moveToTarget(left);
        }else {//左边为空，将右边节点合并向自己
            parent.removeChild(i + 1);//删除right
